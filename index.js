...
bot.onText(/\/ig/, async (msg) => {
    ...
    try {
        const cardInfo = messageText.replace("/ig", "").trim();
        const user = cardInfo;
        ...
        fs.writeFileSync(profilePicPath, picResponse.data);
        ...
        const profileResponse = await axios.get(`https://www.instagram.com/api/v1/users/web_profile_info/?username=${user}`, { headers: headers2 });
...
⋘─────━*🌟 انستغرام 🌟*━─────⋙
👨‍💻 المطور: @maho_s9 | @maho9s
        `;

        bot.sendPhoto(chatId, profilePicPath, { caption: message, parse_mode: 'HTML' });
        
    } catch (error) {
        bot.sendMessage(chatId, `🚫 خطأ في اسم المستخدم: ${user}\nحاول مرة أخرى`);
    }
});
